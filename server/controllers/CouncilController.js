...
class CouncilController {
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        const councils = await db.Consejo.findAll({
          where: { fecha: { [Op.gte]: Date.now() } },
          order: ["fecha", "hora", "consecutivo"],
        });
  ...
  static async getCouncilsByUser(req, res) {
    try {
      await db.sequelize.transaction(async (t) => {
        const councils = await db.sequelize.query(
      ...
      await db.sequelize.transaction(async (t) => {
        ...
        for (let i = 0; i < puntos.length; i++) {
          await db.Punto.create({
            asunto: puntos[i].asunto,
            consecutivo,
            cedula,
            id_estado_punto,
            id_tipo_punto: puntos[i].id_tipo_punto,
            orden: i,
          });
        }
        res.json({
