    ...
    try {
      await db.sequelize.transaction(async t => {
        ...
        for (let i = 0; i < convocados.length; i++) {
          await db.Convocado.create({ cedula: convocados[i], consecutivo, limite_solicitud })
          const correos = await db.Correo.findAll({ attributes: ['correo'], where: { cedula: convocados[i] } })
    ...
    try {
      await db.sequelize.transaction(async t => {
        for (let i = 0; i < req.body.noConvocados.length; i++) {
          ...
          await db.Convocado.destroy({ where: { cedula, consecutivo: req.params.consecutivo } })
        }
        res.json({
          success: true
        })
