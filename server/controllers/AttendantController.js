...
class AttendantController {
  static async getByCouncil(req, res) {
    try {
      await db.sequelize.transaction(async (t) => {
        const convocados = await db.Convocado.findAll({
          attributes: ["cedula"],
  ...
  static async getUserNames(req, res) {
    try {
      await db.sequelize.transaction(async (t) => {
        const convocados = await db.sequelize.query(
          `SELECT "Usuario"."nombre", "Usuario"."apellido", "Usuario"."segundo_apellido", "Usuario"."id_tipo_convocado" FROM public."Usuario" INNER JOIN public."Convocado" ON "Usuario"."cedula" = "Convocado"."cedula" WHERE "Convocado"."consecutivo" = '${req.params.consecutivo}'`
        );
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        ...
        for (let i = 0; i < convocados.length; i++) {
          ...
          const correos = await db.Correo.findAll({
            attributes: ["correo"],
        ...
        if (mailList.length > 0) {
          const consejo = await db.Consejo.findOne({ where: { consecutivo } });
          ...
          const mailOptions = {
            ...
            text: `Los administradores del Consejo de Ingeniería en Computación del Campus Tecnológico Local de San José le han convocado a la sesión ${
              info.id_tipo_sesion === 1
                ? "ordinaria"
                : info.id_tipo_sesion === 2
                ? "extraordinaria"
                : "de Consulta Formal"
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        for (let i = 0; i < req.body.noConvocados.length; i++) {
          const cedula = req.body.noConvocados[i];
          await db.Convocado.destroy({
            where: { cedula, consecutivo: req.params.consecutivo },
          });
        }
        res.json({
          success: true,
