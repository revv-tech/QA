...
class UserController {
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        const users = await db.Usuario.findAll({
          attributes: ["cedula", "nombre", "apellido", "segundo_apellido"],
          order: ["nombre"],
        });
        if (users.length > 0) {
          res.json({
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        ...
        const mailOptions = {
          ...
          text: `Los administradores del Consejo de Ingeniería en Computación del Campus Tecnológico Local de San José han solicitado que seas un participante en el Consejo de Unidad Desconcentrada, por favor haz click en la siguiente dirección y llena los datos solicitados:\n\nhttps://il-consigliere.herokuapp.com/gUsuarios/registro/${token}\n\nSe le recuerda que esta dirección expirará en 3 horas.\n\nSaludos,\nConsejo de Ingeniería en Computación.`,
        };
        transporter.sendMail(mailOptions, (err, resp) => {
          if (err) {
            res.status(500).json({
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        const links = await db.Link.findAll({
          ...
          where: { token, expiration: { [Op.gt]: Date.now() } },
        });
        await db.Link.destroy({ where: { token } });
        if (links.length !== 0) {
          res.json({
            success: true,
            links,
  ...
  static async authenticate(req, res) {
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        const user = await db.Usuario.findOne({ where: { cedula } });
        if (user) {
  ...
  static async update(req, res) {
    ...
    try {
      await db.sequelize.transaction(async (t) => {
        await db.Usuario.update(
          { cedula, nombre, apellido, segundo_apellido },
          { where: { cedula: req.params.cedula } }
        );
        res.json({
          success: true,
  ...
  static async roles(req, res) {
    try {
      db.sequelize.transaction(async (t) => {
        const roles = await db.Usuario_Permiso.findAll({
