...
class UserController {
  ...
  static async update (req, res) {
    ...
    try {
      await db.sequelize.transaction(async t => {
        await db.Usuario.update({ cedula, nombre, apellido, segundo_apellido }, { where: { cedula: req.params.cedula } })
        res.json({
          success: true
        })
      })
  ...
  static verifyToken (req, res) {
    ...
    if (typeof bearerHeader !== 'undefined') {
      ...
      try {
        const decoded = jwt.verify(token, process.env.KEY, { algorithm: 'HS512' })
        res.json({
          success: true,
          token: decoded
        })
  ...
  static async updateAttendantType (req, res) {
    ...
    try {
      await db.sequelize.transaction(async t => {
        await db.Usuario.update({ id_tipo_convocado }, { where: { cedula: req.params.cedula } })
        res.json({
          success: true
        })
      })
